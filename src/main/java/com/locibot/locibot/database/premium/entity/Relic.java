...
public class Relic extends SerializableEntity<RelicBean> implements DatabaseEntity {
    ...
    public Mono<UpdateResult> activate(Snowflake userId, @Nullable Snowflake guildId) {
        return Mono.from(DatabaseManager.getPremium()
                ...
                .doOnSubscribe(__ -> {
                    PremiumCollection.LOGGER.debug("[Relic {}] Activation", this.getId());
                    Telemetry.DB_REQUEST_COUNTER.labels(DatabaseManager.getPremium().getName()).inc();
                })
                .doOnNext(result -> PremiumCollection.LOGGER.trace("[Relic {}] Activation result; {}", this.getId(), result))
    ...
    public Mono<Void> insert() {
        return Mono.from(DatabaseManager.getPremium()
                ...
                .doOnSubscribe(__ -> {
                    PremiumCollection.LOGGER.debug("[Relic {}] Insertion", this.getId());
                    Telemetry.DB_REQUEST_COUNTER.labels(DatabaseManager.getPremium().getName()).inc();
                })
                .doOnNext(result -> PremiumCollection.LOGGER.trace("[Relic {}] Insertion result: {}", this.getId(), result))
                .doOnTerminate(DatabaseManager.getPremium()::invalidateCache)
                .then();
    }
    ...
    public Mono<Void> delete() {
        return Mono.from(DatabaseManager.getPremium()
                ...
                .doOnSubscribe(__ -> {
                    PremiumCollection.LOGGER.debug("[Relic {}] Deletion", this.getId());
                    Telemetry.DB_REQUEST_COUNTER.labels(DatabaseManager.getPremium().getName()).inc();
                })
                .doOnNext(result -> PremiumCollection.LOGGER.trace("[Relic {}] Deletion result: {}", this.getId(), result))
                .doOnTerminate(DatabaseManager.getPremium()::invalidateCache)
                .then();
    }
