	...
	public void onUserVoiceChannelEvent(UserVoiceChannelEvent event) {
		...
		if(botVoiceChannel != null) {
			...
			if(this.isAlone(botVoiceChannel) && !gmm.isCancelling()) {
				BotUtils.sendMessage("Il n'y a plus personne qui écoute de la musique, musique mis en pause, je quitterai le salon dans 2 minutes.", gmm.getRequestedChannel());
				gmm.getScheduler().setPaused(true);
				gmm.scheduleLeave();
			} else if(!this.isAlone(botVoiceChannel) && gmm.isCancelling()){
				BotUtils.sendMessage("Quelqu'un m'a rejoint, reprise de la musique.", gmm.getRequestedChannel());
