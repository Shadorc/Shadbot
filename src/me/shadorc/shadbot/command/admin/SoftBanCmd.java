							...
							return Flux.fromIterable(mentions)
									...
									.concatMap(member -> member.getPrivateChannel()
											...
											.switchIfEmpty(Mono.error(new CommandException(
													String.format("I cannot softban **%s** because he is higher in the role hierarchy than me.", member.getUsername()))))
											.filterWhen(ignored -> DiscordUtils.isUserHigher(guild, context.getMember(), member))
											.switchIfEmpty(Mono.error(new CommandException(
													String.format("You cannot softban **%s** because he is higher in the role hierarchy than you.", member.getUsername()))))
